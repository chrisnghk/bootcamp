...
public interface StockRepository extends JpaRepository<Stock, Long> {
  ...
  // @Query(
  //     value = "select s.id, s.country, case s.company_name when 'Test' then 'TestTestTest' end as company_name, s.logo, s.ipo_date, s.market_cap, s.currency from finnhub_stocks s where s.id = :id",
  //     nativeQuery = true)
  // List<Stock> findAllById2(@Param(value = "id") Long id);
